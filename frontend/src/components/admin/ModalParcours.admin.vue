...
const props = defineProps({
  isOpen: Boolean,
  parcours: Object, // pour l'édition
});
...
watch(
  ...
  (newParcours) => {
    if (newParcours) {
      formData.value = { name: newParcours.name };
    } else {
      resetForm();
    }
  }
...
const handleSubmit = async () => {
  if (!formData.value.name.trim()) {
    error.value = "Le nom du parcours est requis";
    return;
  }
  ...
  try {
    if (props.parcours) {
      await parcoursStore.updateParcours(props.parcours.id_parcours, formData.value);
    } else {
      await parcoursStore.createParcours(formData.value);
    }
      ...
      <div class="bg-white rounded-lg w-full max-w-md mx-auto">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
          <h2 class="text-lg font-semibold text-gray-900">
            {{ parcours ? "Modifier le parcours" : "Créer un nouveau parcours" }}
          </h2>
        </div>
        ...
        >
          ...
          >
            {{ isLoading ? "Enregistrement..." : parcours ? "Modifier" : "Créer" }}
          </button>
        </div>
      </div>
    </div>
